...
This utility function will allow us to add Tailwind CSS classes conditionally without causing any style conflicts. This is also one of the most useful utility function which got popular from [shadcn/ui](https://ui.shadcn.com).

## Define Variants

We will use [Class Variance Authority (CVA)](https://cva.style/docs) to define different variants for the link component. These variants will be Tailwind CSS classes.
...
We are passing three props to this component href, children and classNmae. `className` will take all the variants and any other classes. We are using the `cn` utility function to add classes without any CSS conflicts.
...
We check if `href` is starting with a `https` or `mailto` protocol. This is a simple check, and you can find more robust methods using the hostname, protocol scheme, etc… You can find the full code [here](https://github.com/SameerJadav/sameerjadav.me/blob/main/src/components/Anchor.tsx).
